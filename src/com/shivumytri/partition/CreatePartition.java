...
public class CreatePartition {
	...
	public void createPartition () {
		...
		try {
			...
			//	System.out.println("Enter number of columns, per partition_" + filename + (i + 1) );
	...
	public List<Itemsets> callAprioriAlgorith(ArrayList<File> listoffile, Scanner sc, int noofpartition, String fullname) {
		...
		for (int i = 0; i < listoffile.size(); i++) {
			...
			output = ".\\finaloutput\\"+ inputfile.getName()+"out";
			
			try {
				AlgoApriori apriori = new AlgoApriori();
				//apriori.runAlgorithm(minsup, input, output);				
				Itemsets t =  apriori.runAlgorithm(minsup, input, null);//apriori.runAlgorithm(0.1, output, null);
				apriori.printStats();
				results.add(t);
	...
	public void generateSecondFP(List<Itemsets> results, String outputfilename, String inputfilename, double minsupp, int noofpartition ) {	
	...
	private void getSupportCountofNewItems(List<Itemset> candidatesK, List<int[]> dbdata, int minsupRelative, BufferedWriter writer) {		
			...
			loopCand:	for(Itemset candidate : candidatesK){
				...
				for(int item: transaction){
					...
					// is larger than the one that we search  in candidate.
						...
						//logger.debug("Skipping ......"+item +"::"+ candidate.itemset[pos]);
						continue loopCand;
					}
	...
	private ArrayList<String> getPartitionDataForEachFile(String line, int[] partitiondetails) throws Exception {
		...
		if(totalparitionsize == noofcolumns){
			...
			throw new Exception("Partition Failed : Total partition columns does not added upto total of columns in file.");
		}
		return result;
